...
describe("isoline labels", () => {
  ...
  it("labels a long U/M/E contour that only clips a pinched millimetre window", () => {
    const levels = [3000, 3200, 1.4];
    const uChain = {
      level: 3000,
      pts: Array.from({ length: 81 }, (_, i) => ({ x: 0.25, y: -2 + i * 0.05 })),
    };
    const mChain = {
      level: 1.4,
      pts: Array.from({ length: 81 }, (_, i) => ({ x: 0.31, y: -2 + i * 0.05 })),
    };
    const eChain = {
      level: 3200,
      pts: Array.from({ length: 81 }, (_, i) => ({ x: 0.37, y: -2 + i * 0.05 })),
    };
    const labels = pickIsoLabels([uChain, mChain, eChain], levels, {
      xMin: 0.2,
      yMin: -0.03,
      xMax: 0.4,
      yMax: 0.03,
      toPx: (x, y) => ({ x: 80 + x * 900, y: 140 - y * 900 }),
    });
    expect(labels.length).toBe(3);
    expect(labels.map((l) => l.level).sort((a, b) => a - b)).toEqual([1.4, 3000, 3200]);
    expect(labels.every((l) => l.y >= -0.03 && l.y <= 0.03)).toBe(true);
    expect(labels.every((l) => l.x >= 0.2 && l.x <= 0.4)).toBe(true);
  });
