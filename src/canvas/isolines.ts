...
/** Liang–Barsky clip of a segment to an axis-aligned millimetre window. */
function clipSegToBox(
  a: Xy,
  b: Xy,
  xMin: number,
  yMin: number,
...
/** Visible pieces of a polyline in the current millimetre window. */
export function clipPolyToBox(
  pts: Xy[],
  xMin: number,
  yMin: number,
  xMax: number,
  ...
  const pushPt = (p: Xy) => {
    ...
    if (last && Math.abs(last.x - p.x) <= 1e-12 && Math.abs(last.y - p.y) <= 1e-12) return;
    cur.push(p);
  };
  const flush = () => {
  ...
  for (let i = 1; i < pts.length; i++) {
    const clipped = clipSegToBox(pts[i - 1], pts[i], xMin, yMin, xMax, yMax);
    if (!clipped) {
      flush();
      continue;
    }
    if (!cur.length) pushPt(clipped[0]);
    else {
      const last = cur[cur.length - 1];
      if (Math.abs(last.x - clipped[0].x) > 1e-9 || Math.abs(last.y - clipped[0].y) > 1e-9) {
        flush();
        pushPt(clipped[0]);
      }
    }
 ...
 * Candidates are taken from the visible clip of each chain so a long U / M / E
 * contour still gets a number when the millimetre window is pinched in.
 * One label per visible piece; a level’s y-mirror is labelled only when it does not collide.
 * Soft cap ~12 — not a hard 5.
...
export function pickIsoLabels(
  ...
  const fracs = [0.22, 0.32, 0.42, 0.52, 0.62, 0.72, 0.18, 0.82, 0.28, 0.58, 0.68, 0.38, 0.5];
  const maxLabels = 12;
  ...
  for (const level of levels) {
    ...
    const pieces = chains
      ...
      .flatMap((c) => clipPolyToBox(c.pts, opts.xMin, opts.yMin, opts.xMax, opts.yMax))
      .map((pts) => ({ pts, L: chainLength(pts) }))
      .sort((a, b) => b.L - a.L);
    for (const { pts, L } of pieces) {
      if (labels.length >= maxLabels) break;
      if (L < 1e-4) continue;
